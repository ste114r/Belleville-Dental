...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($new_password !== $confirm_password) {
        ...
        // 4. Verify Current Password using a secure prepared statement
        $stmt = mysqli_prepare($con, "SELECT password_hash FROM USERS WHERE user_id = ?");
        ...
        if ($user && $current_password === $user['password_hash']) {
            ...
            // 5. Update Password in the Database using a prepared statement
            $stmt_update = mysqli_prepare($con, "UPDATE USERS SET password_hash = ? WHERE user_id = ?");
